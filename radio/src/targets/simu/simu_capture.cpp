 ...
 * This file is compiled only into the interactive `simu` executable, which
 * installs the encoder via simuSetCaptureDumpFn() before the main loop starts.
 *
...
struct CaptureBackendInstaller {
  CaptureBackendInstaller() { simuSetCaptureDumpFn(captureDump); }
} captureBackendInstaller;
