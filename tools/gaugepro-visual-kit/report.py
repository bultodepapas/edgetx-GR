...
def _provenance(rows):
    captures = [r for r in rows if r.get("file")]
    values = {}
    for key in ("run_id", "git_sha", "source_sha", "harness_sha", "simu_sha"):
        found = sorted({r.get(key) for r in captures if r.get(key)})
        values[key] = found[0] if len(found) == 1 else "mixed"
    values["fresh"] = sum(1 for r in captures if r.get("fresh"))
    values["retained"] = len(captures) - values["fresh"]
    return values


def _append_short_provenance(lines, rows):
    p = _provenance(rows)
    lines.append("Evidence run: `%s`; source `%s`; harness `%s`; simulator `%s`." %
                 (p["run_id"], p["source_sha"][:12], p["harness_sha"][:12],
                  p["simu_sha"][:12]))
    lines.append("")
...
def write_catalog_md(path, rows, skipped):
    ...
    lines.append("")
    _append_short_provenance(lines, rows)
...
def write_run_summary(path, rows, simu_build=""):
    ...
    lines.append("- Run ID: `%s`" % provenance["run_id"])
    lines.append("- Git commit: `%s`" % provenance["git_sha"])
    lines.append("- Runtime source SHA-256: `%s`" % provenance["source_sha"])
    lines.append("- Visual harness SHA-256: `%s`" % provenance["harness_sha"])
    lines.append("- Simulator SHA-256: `%s`" % provenance["simu_sha"])
    lines.append("- Provenance: %d fresh, %d retained capture(s)" %
                 (provenance["fresh"], provenance["retained"]))
    if simu_build:
