# EdgeTX Gauge V2 — Reality-Adjusted Development Plan
...
**Target:** Gauge V2 prototype 0.1 — Lua LVGL widget
...
| **B. Dev showcase PR** | `EdgeTX/edgetx-sdcard` → `dev/WIDGETS/GaugeV2/` | The only EdgeTX-org repo that accepts Lua widgets (precedent: `dev/WIDGETS/Counter`). Optional, maintainer acceptance not guaranteed |
...
- Branch `feat/gauge-v2` created from `origin/main` (7c07b6df7) — deliberately **not** from the
  unrelated `fix/lua-float-integer-equality` HEAD, so the future PR contains only Gauge V2 work.
...
Placement rationale: identical to the on-radio path (`/WIDGETS/GaugeV2/main.lua`), so the
...
- Performance: 4 simultaneous Gauge V2 widgets, no perceptible slowdown (draft §31).
