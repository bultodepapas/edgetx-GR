 # Gauge V2 — Senior Improvement Plan (v2.0)
...
paints with the legacy `lcd` API. **GaugeV2 would be the first vector
...
2. **The design is a wireframe, not an instrument.** Flat line needle, solid
   ...
   triangle needle**; GaugeV2 currently looks *less* finished than either.
...
4. **`main.lua` is loaded for every widget at radio boot**, used or not
   (rotorflight's RfStats states this explicitly). GaugeV2's 7 KB `main.lua`
...
### 2.1 GaugeV2 is the first LVGL vector instrument — plan accordingly
...
- The official examples use `lvgl.build` trees with **function-valued
  ...
  `callRefs`. GaugeV2's explicit `lvgl.set`-on-change is cheaper. Keep it, and
...
`disconnected`). GaugeV2's mock serves static values only, which is why
...
- *"Rotary Gauge stays at -1 regardless of source"* — the auto-scale sentinel
  colliding with a real value. GaugeV2's equivalent trap is "presets apply only
...
| Capability | Where | Use for Gauge V2 |
|---|---|---|
...
fork's binding normalises 405→45, so the absolute form GaugeV2 uses is a
...
chars** (`GaugeV2` ✓), **option names ≤ 10 chars, no spaces** — long labels
...
| §1 | "successor to GaugeRotary" | GaugeRotary was never read at source level; §2 lists five features it has that GaugeV2 does not |
