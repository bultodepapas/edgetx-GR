...
Gauge Dial Pro and Gauge Bar Pro are **first-public-beta candidates**, not a
final release. Back up the radio SD card and model configuration before
installing them, and do not use a widget as the only warning for a
...
The current readiness verdict and remaining release gates are recorded in the
[`2026-08-13 beta/pre-PR review`](docs/PRE-PR-BETA-REVIEW-2026-08-13.md).
...
simulator build. The fresh 277-screen simulator run ended with no runtime
failures, but its duplicate-frame audit found two unexpected groups caused by
dropped gallery navigation events. The product runtime is a beta candidate;
the visual evidence gate remains open until that harness issue is fixed and a
clean capture is produced. See the
