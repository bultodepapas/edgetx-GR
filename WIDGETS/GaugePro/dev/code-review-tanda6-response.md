...
in 14 files**, and — importantly — **zero global writes**, so GaugePro cannot
...
sample) restores the contract but keeps GaugePro measuring text to position
...
**That single invariant is F-1 and F-5 at once.** GaugePro's build/update split
is the deviation.

The model to copy is not `text.cpp` (too dumb — GaugePro's signature gate is a
...
which has exactly GaugePro's architecture plus the piece GaugePro is missing:
...
GaugePro has steps 2 and 3 (`layout.signature()`) and **no step 1**. The
...
styles, and GaugePro's `frame.colorKey` + `setProp` is the correct Lua
...
displayed state never changed. GaugePro's colour key encodes only the semantic
...
GaugePro's `lvgl.clear()` + full rebuild ([app.lua:198-201](../app.lua#L198-L201))
...
width differs (126-134). GaugePro's `frame.*` + `setProp` is the same idea,
better factored.
...
- `getPt` reads coordinates with `luaL_checkunsigned`
  ...
  **a negative coordinate raises a Lua error** — F-1 again, from a third door.
  ...
  sweeps.
- The firmware already dedupes identical point sets, so GaugePro's
...
- **`lastValue = -10000` sentinels** (`gauge.cpp:103`, `value.cpp:172`).
  GaugePro's `nil` + explicit availability model is strictly better; a magic
...
- **Rebuilding children on every geometry change** (`OutputsWidget`).
  GaugePro's signature gate is the improvement; keep it.
- **`Messaging::send` fan-out per frame** (`outputs.cpp:234`). No Lua
  equivalent, no reason to want one.

### C.8 Static analysis: `.luacheckrc`

A `luacheck` config was added at `WIDGETS/GaugePro/.luacheckrc`. Two decisions
...
Toolchain is installed and verified (§A). From `WIDGETS/GaugePro/`:
...
- **B (correct, §C.1):** give `configure()` the *unconditional prelude* that
  `OutputsWidget::update()` has and GaugePro lacks — re-apply non-structural
